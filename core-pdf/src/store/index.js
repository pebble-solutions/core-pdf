...
export default createStore({
	...
	mutations: {
		 ...
		 * Remplace la liste des éléments chargés avec une nouvelle liste
		 * @param {Object} state		Le state de l'instance VueX
		 * @param {Array} operations		La nouvelle liste d'éléments
		 */
		replaceOperations(state, operations) {
			state.elements = operations;
		},
		...
		updateOperation(state, operations) {
			operations.forEach(operation => {
				let stateOp = state.operations.find(e => e.id === operation.id);

				// Mise à jour d'un élément existant
				if (stateOp) {
					for (let key in operation) {
						stateOp[key] = operation[key];
					}
				}
				// Ajout d'un élément existant
				else {
		...
		removeOperation(state, operations) {
			operations.forEach(id => {
				let index = state.operations.findIndex(e => e.id === id);
	...
	actions: {
		...
		refreshOperations(context, payload) {
			...
			if (payload.action == 'update') {
				//console.log(payload)
				context.commit('updateOperation', payload);
			}
		...
		refreshFichier(context, payload) {
			if (!('action' in payload)) {
				payload.action = 'update';
			}
